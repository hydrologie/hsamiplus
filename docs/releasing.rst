...
The new version based off of the version checked out will now be available via `pip` (`pip install hsamiplus`).
 ...
 * If possible, configure tests within the conda-forge build CI (e.g. `imports: hsamiplus`, `commands: pytest hsamiplus`).
...
From the hsamiplus source folder we can enter into the docker container, providing access to the `src/hsamiplus` source files by linking them to the running image:

    .. code-block:: console

        sudo docker run --rm -ti -v $(pwd):/src/hsamiplus -w /src/hsamiplus quay.io/pypa/manylinux_2_24_x86_64 bash
...
This will then place two files in `hsamiplus/dist/` ("hsamiplus-1.2.3-py3-none-any.whl" and "hsamiplus-1.2.3.tar.gz").
