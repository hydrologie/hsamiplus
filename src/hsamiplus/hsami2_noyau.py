...
def hsami2_noyau(projet,etat):
    ...
    '''
     ...
     Developpé par J.L. Bisson, et F. Roberge en Matlab, 1983
     ...
     ENTRéES
     -------
     projet['nb_pas_par_jour']    Nombre de pas de temps par jour (scalaire)
     projet['pas']                Pas à l'intérieur de la journée (scalaire)
     projet['date']               Date de la journée de la simulation (format datevec, vecteur) 
     projet['superficie']         superficie(1) : Superficie du bassin versant, incluant le réservoir (km^2, scalaire)
                                  superficie(2) : Superficie moyenne du réservoir (= 0 si non modélisé) (km^2, scalaire)
     projet['memoire']            Mémoire utilisée pour les hydrogrammes unitaires (j, scalaire)
     ...
     projet['meteo']['bassin']  Vecteur météo :    tmin [cm]
                                                   tmax [cm]
                                                   pluie [cm]
                                                   neige [cm]
     ...
     projet['meteo']['reservoir']    ** S'il n'existe aucune météo au réservoir, celle
                               du bassin est utilisée. **
                               Vecteur météo :     tmin [cm]
                                                   tmax [cm]
                                                   pluie [cm]
                                                   neige [cm]
                                                   soleil [fraction] (optionnel) 
     ...
     projet['modules']            Structure spécifiant les modules é utiliser
                                  ...
                                  qbase :          'hsami' (défaut)
                                                   'dingman'
                                  radiation :      'hsami' (défaut)
                                                   'mdj'
                                  mhumide:          0  (défaut)
                                                    1
                                  reservoir :       0 (défaut)
                                                    1
                                  glace_reservoir : 0 (défaut)
                                                    'stefan' 
                                                    'mylake'
    
     projet['physio']          Structure contenant des variables
                               physiographiques du bassin
                               ** Toutes les variables physiographiques peuvent
                               étre facultatives, dépamment des modules
                               utilisés[' **
     ...
     etat                     états du bassin versants et du réservoir
                             ...
                             - etat['mdj']['couvert_neige']           : Hauteur du couvert de neige dans les 2 ou 3 milieux (m, vecteur de la même taille que physio.occupation)
                             ...
                             - etat['alt']['couvert_neige']           : Hauteur du couvert de neige dans les 2 ou 3 milieux (m, vecteur de la même taille que physio.occupation_bande)
                             - etat['alt']['densite_neige']           : Densité du couvert de neige (fraction décimale, vecteur de la même taille que physio.occupation_bande)
                             ...
                             - etat['ratio_bassin']                : Fraction de la superficie occupée par la partie terrestre du bassin versant (fraction décimale, scalaire)
                             - etat['ratio_reservoir']             : Fraction de la superficie occupée par le réservoir (fraction décimale, scalaire)
                             - etat['ratio_fixe']                  : Fraction de la superficie occupée par la partie terrestre pour le calcul des processus souterrains (fraction décimale, scalaire) 
     
    
     SORTIES
     -------
     etat                     (même structure qu'en entrée contenant les valeurs mises à jour)
     s                         s['Qtotal']
                               s['Qbase']
                               s['Qinter']
                               s['Qsurf']
    ...
    etats_ini = etat['ratio_bassin'] * etat['neige_au_sol'] + etat['ratio_fixe'] * (etat['gel'] + etat['nappe'] + np.nansum(etat['sol']) + \
                                                                                    etat['mhumide']) + np.nansum(etat['eeg'])/superficie[0] 

    reserv_ini = etat['ratio_fixe'] * etat['reserve'] 
    eaux_hu_ini = etat['ratio_fixe'] * (np.nansum(etat['eau_hydrogrammes'][:,0] + etat['eau_hydrogrammes'][:,2])+ np.nansum(etat['eau_hydrogrammes'][0:9,1])) 
    ...
    # ETP au bassin
    etps = [0, 0]  # Define the missing variable "etps"
    etps[0] = hsami_etp(pas, nb_pas, jj, meteo['bassin'][0], meteo['bassin'][1], modules['etp_bassin'], physio)

    # ETP au réservoir
    etps[1] = hsami_etp(pas, nb_pas, jj, meteo['reservoir'][0], meteo['reservoir'][1], modules['etp_reservoir'], physio)
    ...
    if modules['glace_reservoir'] == 'stefan' or modules['glace_reservoir'] == 'mylake':
        ...
        raise ValueError('L''option spécifiée pour la modélisation de la glace de réservoir est invalide')
    ...
    apport_vertical,etat,etr = hsami_ecoulement_vertical(nb_pas,param,etat,infiltration,demande_eau,modules,ruissellement,apport_vertical,etr) 
    ...
    if 'hu_surface' in projet:
        if len(projet['hu_surface']) != projet['memoire']:
            print('L''hydrogramme unitaire de surface imposé doit avoir la même durée que la variable mémoire')
    ...
    if 'hu_inter' in projet:
        if len(projet['hu_inter']) != projet['memoire']:
            print('L''hydrogramme unitaire intermédiaire imposé doit avoir la même durée que la variable mémoire')

        hydrogrammes[:, 1] = projet['hu_inter']  # Note : Les paramétres 21 et 22 ne seront pas utilisés
    else:
        hydrogrammes = np.vstack((hydrogrammes, hsami_hydrogramme(param[21], param[22], nb_pas, projet['memoire'] / nb_pas))).T  # hu inter
    ...
    bilan['horizontal']['etat'][0] = np.sum(etat['eau_hydrogrammes'][:, 0] + etat['eau_hydrogrammes'][:, 2]) + \
                                 np.sum(etat['eau_hydrogrammes'][0:9, 1]) + etat['reserve']

    apport_horizontal, etat['reserve'], etat['eau_hydrogrammes'] = hsami_ecoulement_horizontal(nb_pas, param[18], 
                            etat['reserve'], etat['eau_hydrogrammes'], hydrogrammes, apport_vertical, modules)
    ...
    etats_bilan = etat['ratio_bassin']*etat['neige_au_sol'] + etat['ratio_fixe']*(np.nansum(etat['sol']) + etat['gel'] + \
                  etat['nappe'] + etat['mhumide']) + np.nansum(etat['eeg'])/superficie[0]
    ...
    # ---------------------------------
    # Glace
    delta['glace'] = bilan['glace']['entrees'] - bilan['glace']['sorties'] + \
                  bilan['glace']['etat'][0] - bilan['glace']['etat'][1]

    # Interception
    delta['interception'] = bilan['interception']['entrees'] - bilan['interception']['sorties'] + \
                         bilan['interception']['etat'][0] - bilan['interception']['etat'][1]

    # Ruissellement
    delta['ruissellement'] = bilan['ruissellement']['entrees'] - bilan['ruissellement']['sorties'] + \
                          bilan['ruissellement']['etat'][0] - bilan['ruissellement']['etat'][1]

    # Ecoulement vertical
    delta['vertical'] = bilan['vertical']['entrees'] - bilan['vertical']['sorties'] + \
                     bilan['vertical']['etat'][0] - bilan['vertical']['etat'][1]

    # Milieu humide
    if modules['mhumide'] == 1:
        delta['mhumide'] = bilan['mhumide']['entrees'] - bilan['mhumide']['sorties'] + \
                        bilan['mhumide']['etat'][0] - bilan['mhumide']['etat'][1]
    else:
        delta['mhumide'] = 0.0 if nb_pas == 1 else [0.0]*nb_pas.tolist()

    # Ecoulement horizontal
    delta['horizontal'] = bilan['horizontal']['entrees'] - bilan['horizontal']['sorties'] + \
                       bilan['horizontal']['etat'][0] - bilan['horizontal']['etat'][1]
