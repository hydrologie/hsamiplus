...
def hsami_ecoulement_vertical(nb_pas,param,etat,offre,demande,modules,ruissellement_surface,apport_vertical,etr):
    '''
     ENTRÉES
     -------
     ...
     offre                  : Quantité d'eau disponible pour l'évaporation et le
                             ruissellement (cm)
     demande    	        : Demande évaporative de l'atmosphére (cm)
     modules                : Voir hsami2_noyau (projet['modules'])
     ruissellement_surface  : Ruissellement (cm)
     apport_vertical        : Lames d'eau verticales (voir hsami_interception)
     etr                    : Composantes de l'évapotranspiration (voir hsami_interception)
     ...
     EXEMPLE
     -------
     test_ecoulement_vertical
     Tous les modules sont par DéFAUT, sauf ceux é tester.
     Les modules d'infiltration sont testés avec modules.sol = 'hsami'.
     Les modules de qbase sont testés avec modules.sol = '3couches'.
    
    '''

    if modules['sol'] == '3couches':
    
        [apport,etat,etr] = ecoulement_3couches(nb_pas,param,etat,offre,demande,modules,ruissellement_surface,apport_vertical,etr)
        ...
        # ----------------------------------
        apport,nappe,sol = vidangeNappe(apport,nappe,taux_vidange_nappe,nappe_max,nb_pas,modules,param,sol)
...
def ecoulement_3couches(nb_pas,param,etat,offre,demande,modules,ruissellement_surface,apport_vertical,etr):
    """
    Calcule l'écoulement vertical dans un système à trois couches de sol.

    Entrées:
        nb_pas (int): Le nombre de pas de temps.
        param (list): La liste des paramètres du modèle.
        etat (dict): Le dictionnaire des états du système.
        offre (float): L'offre en eau disponible.
        demande (float): La demande en eau.
        modules (dict): Le dictionnaire des modules utilisés.
        ruissellement_surface (float): Le ruissellement en surface.
        apport_vertical (list): La liste des apports verticaux.
        etr (list): La liste des évapotranspirations.

    Sorties:
        tuple: Un tuple contenant les apports, les nouveaux états et les évapotranspirations.
    """
    ...
    # Identification des variables d'état
    # -----------------------------------
    sol = [etat['sol'][0], etat['sol'][1], etat['nappe']] # La nappe est ajoutée au bas de la colonne de sol pour simplifier les calculs
    gel = etat['gel']
    neige_au_sol = etat['neige_au_sol']
    ...
    if ecart_offre_demande > 0:
        ...
        if modules['infiltration'] == 'green_ampt':
            psi = param[25]
            inf_potentielle, apport[2] = green_ampt(offre, param[34], psi, sol_max[0], sol[0], nb_pas, gel, neige_au_sol, n[0])
    ...
    for i_p in range(0, pas_1h):
        ...
        # Calcul de K
        K = [Ks[0]*(sol[0]/sol_max[0])**c[0], Ks[1]*(sol[1]/sol_max[1])**c[1]]
        
        # Drainage gravitaire potentiel de chaque couche
        drainage = [sol_max[0]*K[0]*(1/24)/z[0], sol_max[1]*K[1]*(1/24)/z[1]]
...
# Vidange et débordement de la nappe
def vidangeNappe(apport, nappe, taux_vidange_nappe, nappe_max, nb_pas, modules, param, sol):
    """
    Effectue la vidange d'une nappe phréatique en fonction des paramètres donnés.

    Entrées:
        apport (list): Liste contenant les apports d'eau.
        nappe (float): Niveau de la nappe phréatique.
        taux_vidange_nappe (float): Taux de vidange de la nappe phréatique.
        nappe_max (float): Niveau maximum de la nappe phréatique.
        nb_pas (int): Nombre de pas de temps.
        modules (dict): Dictionnaire contenant les modules utilisés.
        param (list): Liste contenant les paramètres du modèle.
        sol (float): Sol du modèle.

    Sorties:
        tuple: Tuple contenant les nouveaux apports d'eau, le nouveau niveau de la nappe phréatique et le sol du modèle.
    """
    # Vidange
    if modules['qbase'] == 'hsami':
...
def green_ampt(eau_surface, Ks, psi, sol_max, sol, nb_pas, gel, neige_au_sol, *args):
    ...
    =========================================================================
     Fonction calculant l'infiltration et le ruissellement selon le modéle de
     Green-Ampt tel qu'implémenté dans SWAT
    
    
     Entrées :  eau_surface   Eau disponible en surface aprés avoir comblé la
                              demande évaporative (cm)
                Ks            Conductivité hydraulique saturée (cm/j)
                psi           Pression matricielle au front mouillant, dérivée
                              de Rawls (1993) (cm)
                sol_max       Paramétre 13 correspondant au volume max d'eau
                              ds le sol (cm)
                sol           Variable d'état correspondant au volume d'eau
                              dans le sol (cm)
                nb_pas        Nombre de pas de temps dans une période de 24h
                              (entier positif)
                gel           Gel dans la premiére couche de sol (cm)
                neige_au_sol  équivalent en eau de la neige au sol (cm)
                *args         Porosité de la premiére couches de sol si
                              3couches est utilisé (cm3/cm3)
    
     Sorties :  infiltration selon Green-Ampt
                ruissellement
...
def scs_cn(eau_surface, cn):
    ...
    # Entrées :  eau_surface   Eau disponible en surface aprés avoir comblé la
    #                          demande évaporative (cm)
    #            cn            Curve Number (paramétre 24)
    #
    # Sorties :  infiltration
    #            ruissellement
