...
def hsami_interception(nb_pas,jj,param,meteo,etp,etat,modules,physio):
    '''
     ...
     meteo            : Voir hsami2_noyau (projet['meteo'])
     etp              : évapotranspiration potentielle du pas de temps (bassin
                        et réservoir)
     ...
     SORTIES
     -------
     eau_surface       : Eau disponible à la surface pour évaporation,
                         ruissellement et infiltration
     demande_eau       : Demande en eau restante
     etat              : Voir hsami2_noyau
     etr[0]            : Sublimation de la neige
     etr[1]            : évaporation de la pluie sur la neige et de l'eau de fonte
     etr[2]            : évapotranspiration de la pluie
     etr[3]            : Eau pompée par la végetation
     etr[4]            : évaporation directe au réservoir
     apport_vertical   : Vecteur représentant les lames d'eau à moduler par
                         les hydrogrammes unitaires (sauf 4 et 5)
    							1: débit de base
    							2: apport intermédiaire
    							3: ruissellement de surface
                               4: précipitations directes au réservoir
                               5: fonte de la glace
    
     EXEMPLE
     -------
     Ex1. : pas de temps 50 de projet_test.json
     Tous les modules sont ceux PAR DÉFAUT, sauf les modules d'ÉEN à tester
     ('hsami','dj','mdj','alt')
    ...
    # -----------------------------------
    # Variables d'entrées météorologiques
    # -----------------------------------
    ...
    if modules['een'] == 'hsami' or modules['een'] == 'dj':
        ...
        if dt_max < 0:
            ...
            if neige_au_sol > 0.0254:
                ...
                if fonte > 0:
                    eau_fonte, neige_au_sol, neige_au_sol_totale, fonte, fonte_totale = percolation_eau_fonte(neige_au_sol, neige_au_sol_totale, fonte, fonte_totale)
        ...
        if modules['een'] == 'mdj':
            for i_z in range(n):
                taux_de_fonte[i_z] = param[27+i_z]/100      # Taux de fonte - milieu(i_z) # /100 pour cm --> m/degC/jour
                temperature_de_fonte[i_z] = param[30+i_z]   # Température de fonte - milieu(i_z) # degC
        ...
        nas_moy = np.sum([a*b for a,b in zip(etat[modules['een']]['neige_au_sol'][0:n],occupation)]) # nas_moy sert seulement pour la maj de l'een.
        ...
        for i_z in range(n):
            ...
            if modules['een'] == 'alt':
                ...
                pluie, neige = pluieneige(t_min, t_max, pluie+neige)
            ...
            # é la sixiéme colonne de la météo, on peut retrouver un
            ...
            # avant la maj.
                if nas_moy != 0:
                    facteur_maj = (meteo['bassin'][5]/100) / nas_moy
                else:
                    facteur_maj = 1
            ...
            if dt_max <= 0:
                sol, gel = gel_sol(duree, dt_max, sol_min, sol, gel, neige_au_sol*100)  # *100 car Neige au sol doit étre en cm.
            ...
            if neige_au_sol > 0 or neige > 0:
                ...
                drel = CalculDensiteNeige(tmoy) / rho_w
                ...
                if tmoy < temperature_de_fonte[i_z]:
                    ...
                    alpha = ConductiviteNeige(dennei * rho_w) / (dennei * rho_w * capacite_thermique_massique_eau_solide)
                ...
                # if isfield(physio,'latitude') && isfield(physio,'i_orientation_bv') && isfield(physio,'pente_bv') && duree == 1 
                # Si le pas de temps est inférieur à 24, il faudrait que l'indice de radiation connaisse l'heure de la journée. à implanter éventuellement.
                ...
                if modules['radiation'] == 'mdj':
                    ...
                    indice_radiation = IndiceRadiation(jj, physio['latitude'], physio['i_orientation_bv'], pas_de_temps, physio['pente_bv'])
                    
                elif modules['radiation'] == 'hsami':
                    # Si les caractéristiques physiographiques du bassins ne sont
                    # pas entrées dans la fonction, l'indice de radiation est
                ...
                albedo_neige = AlbedoEEN(albedo_neige,drel,neige_au_sol,neige,pas_de_temps,pluie,tneige)
                ...
                dennei = neige_au_sol / couvert_neige # cette valeur peut étre tres élevée quand de la pluie a été ajoutée é la neige_au_sol, la dennei est réajustée aprés.
                
                # Correction de la densité si elle dépasse la densité maximale (survient principalement lorsque de la pluie a été ajoutée au couvert de neige)
                        ...
                        for i_g in range(len(eeg)):
                            if eeg[i_g] > 0:
                                ...
                                if tmoy_glace < temperature_de_fonte[i_z]:
                                    ...
                                    erf = Erf((eeg[i_g]/denglace) / (2 * np.sqrt(alpha * pdts)))
                                ...
                                if tmoy_glace > temperature_de_fonte[i_z]:
                                    potentiel_fonte = 1.5 * taux_de_fonte[i_z] * duree * (tmoy_glace - temperature_de_fonte[i_z]) * indice_radiation * (1-albedo_glace)
                                else:
                                    potentiel_fonte = 0
            ...
            etat[modules['een']]['energie_neige'][i_z] = energie_neige  # Ex1.: [-9.8534e+05 -1.0128e+06 -1.0128e+06] [-1.8817e+06 -1.8246e+06 -1.7680e+06 -1.6984e+06 -1.6334e+06]
        ...
        neige_au_sol = np.sum([a*b for a,b in zip(etat[modules['een']]['neige_au_sol'][0:n],occupation)]) # Ex1.:  0.0653   0.1023
        fonte = np.sum([a*b for a,b in zip(etat[modules['een']]['fonte'][0:n],occupation)])  # Ex1.:  0        0

        sol = np.sum([a*b for a,b in zip(etat[modules['een']]['sol'][0:n],occupation)])      # Ex1.:  2.4892   0.7846
        gel = np.sum([a*b for a,b in zip(etat[modules['een']]['gel'][0:n],occupation)])                         # Ex1.:  0.0392   0.0209
...
def percolation_eau_fonte(neige_au_sol, neige_au_sol_totale, fonte, fonte_totale):
    ...
    Percolation de l'eau de fonte dans la neige
    ===========================================
    '''
    # eau en trop dans la neige
...
def IndiceRadiation(jour,latitude,i_orientation_bv,pas_de_temps,pente):
    '''
    =================================================
    ...
    tanOrientation = [0, np.pi/4, np.pi/2, 3*np.pi/4, np.pi, 5*np.pi/4, 3*np.pi/2, 7*np.pi/4, 2*np.pi] #E, NE, N, NO, O, SO, S, SE
    orientation = tanOrientation[i_orientation_bv-1]
    ...
    if t1_hor_sim > t2_hor_sim:
        ...
        i_j1 = 3600 * i_e2 * ((t2_hor_sim - t1_hor_sim) * np.sin(theta) * np.sin(decli) + \
    ...
    if t1_pte_sim > t2_pte_sim:
        ...
        i_j2 = 3600 * i_e2 * ((t2_pte_sim - t1_pte_sim) * np.sin(theta1) * np.sin(decli) + \
...
def AlbedoEEN(albedo,drel,een,neige,pas_de_temps,pluie,tneige):
...
def CalculDensiteNeige(temperature):
...
def pluieneige(tmin,tmax,prec):
    '''
    PLUIENEIGE Séparation de la précipitation en pluie et neige.
    [PLUIE NEIGE] = PLUIENEIGE(tmin,tmax,prec) sépare la
    précipitation en pluie et neige selon l'algorithme suivant:
    Lorsque la valeur moyenne de tmin et tmax est inférieure à -2 deg
    C, la précipitation est complétement transformée en neige.
